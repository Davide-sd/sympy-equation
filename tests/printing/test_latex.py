"""
These tests comes from SymPy's 1.14.0, sympy.printing.tests.test_latex.py

Here, the only edit made are:

* replace sympy's `latex` function with sympy_equation's `extended_latex`.
* replace sympy's `multiline_latex` function with 
  sympy_equation's `multiline_latex`.
"""
...
def multiline_latex(*args, environment="align*", **kwargs):
    return ml_latex(*args, environment=environment, **kwargs)
